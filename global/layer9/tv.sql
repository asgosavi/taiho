...
tv_tracker AS (    
            SELECT replace(studyid,'-','_')     as   studyid   , visit_number     as      visitnum, visit, visitdy as "visitdy", window_before_visit_day as "visitwindowbefore", window_after_visit_day as "visitwindowafter"
            FROM tv_internal.tv_tracker_list where studyid in (select studyid from included_studies)),
...
all_visits AS (
	...
	FROM sv 
	WHERE (studyid, visit) NOT IN (SELECT DISTINCT studyid, visit FROM tv_tracker)
	...
	AND (studyid, visit) NOT IN (SELECT studyid, visit FROM tv_tracker)
